...
public class AugmentedScript : MonoBehaviour
{
	...
	IEnumerator GetCoordinates()
	{
		...
		while (true) {
			...
			if (!Input.location.isEnabledByUser) {
				...
				distanceTextObject.text = "location not enabled";
			...
			/*while (Input.location.status == LocationServiceStatus.Initializing && maxWait > 0) {
				...
				distanceTextObject.text = "LO state : "+Input.location.status+"  "+maxWait;
			...
			/*if (maxWait < 1) {
				...
				distanceTextObject.text = "Timed out : "+maxWait;
				print ("Timed out");
				yield break;
			}*/
	...
	public void Calc(float lat1, float lon1, float lat2, float lon2)
	{
		...
		distanceTextObject.text = "Distance: " + distance;
